...
let self = module.exports = {
    ...
    readArticle: async function (req, res) {
        ...
        if (getTitle.length == 0) {
            response.NOTFOUND(res, {status: 'failed', message: 'article tidak ada', data: []})
        }else {
            response.OK(res, {status: 'success', message: 'muncul', data: getTitle})
        }
    },
    ...
    deleteArticle: async function (req, res) {
        ...
        if (getArticle.length > 0) {
            ...
            response.OK(res, {status: 'success', message: 'artikel berhasil dihapus', data: getArticle})
        }else {
            response.NOTFOUND(res, {status: 'failed', message: 'artikel tidak ada', data: []})
        }
    }
}