...
let self = (module.exports = {
  addCart: async function (req, res) {
    ...
    const findCart = getCart.find(
      (el) => el.product_id === product_id && el.user_id === userId
    );
    // console.log(findCart, 'FIND CART');
    if (!findProduct) {
      //validasi  product_id tidak ada dalam database
  ...
  getCartData: async function (req, res) {
    ...
    const getCart = await query.leftJoin(
      "cart",
      "product",
      "product_id",
      "product_id",
